...
\begin{table}[!htbp]
  \centering
  \caption{Cost of the four fusion operators. Fusion parameters counts only the operator; total counts the whole model. Epochs and training time are means over the five fixed-partition seeds, on one NVIDIA RTX 4060 Laptop GPU. Latency is the median over a single window, measured on the trip-instance graph.}
...
\begin{table}[!htbp]
  \centering
  \caption{Standardised effect sizes over the 39 trips containing at least one positive window. Cohen's $d$ is computed on the paired per-trip differences. The final column counts trips favouring the first model against the second, with ties omitted.}
